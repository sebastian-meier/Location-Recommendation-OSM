...
function individualQuery(min, max, result){
    if(result > amount && result < amount+amount*tolerance){
        ...
        loop++;
        //SELECT COUNT(*) FROM venues, venues_w_categories, neighborhoods WHERE venues_w_categories.venues_id = venues.id AND neighborhoods.venues_id = venues.id AND bb = 1 AND venues_w_categories.cat_52 = 1 AND neighborhoods.cat_52 > 35
...
function nextThreestep(){
    ...
    if(count >= combis.length){
        ...
        if(ci<iterations){
            ...
            fs.writeFile('clusterPerformance.v2.json', JSON.stringify(json), function (err) {
