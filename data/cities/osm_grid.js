...
//green, water, road_l1, train

var ptype = "road_l1";

function processLocations(){
    client.query("SELECT id, ST_Y(geom) AS latitude, ST_X(geom) AS longitude, "+ptype+" AS val FROM grid_osm WHERE "+ptype+" = 999999 LIMIT 1", [], function(err, result) { // OFFSET "+loc_count
        if(err){console.log((new Date()), 'error running query', err);}else{
                ...
                switch(ptype){
                    ...
                    case "water":
                        ...
                        for(var i = 0; i<waters.length; i++){
                            ...
                            waters_query += "\""+waters[i][0]+"\" = '"+waters[i][1]+"'";
                        }

                        var query = "SELECT "+
                                    " ST_Distance(Geography(ST_Transform(ST_SetSRID(way, 900913),4326)), Geography(ST_Transform(ST_SetSRID(ST_MakePoint("+result.rows[0].longitude+","+result.rows[0].latitude+"),900913),4326))) AS dist "+
                                    " FROM planet_osm_polygon WHERE ( "+
                                    waters_query+
                                    " ) AND way IS NOT NULL "+
                                    " ORDER BY ST_Distance(Geography(ST_Transform(ST_SetSRID(way, 900913),4326)), Geography(ST_Transform(ST_SetSRID(ST_MakePoint("+result.rows[0].longitude+","+result.rows[0].latitude+"),900913),4326))) LIMIT 1";
                        ...
                        var query = "SELECT grid_osm.id AS gid FROM grid_osm, planet_osm_polygon WHERE way IS NOT NULL AND ST_Contains(way, geom) AND ("+
                                    waters_query+" ) ";

                        client.query(query, [], function(err, result){
                            if(err){console.log((new Date()), 'error running query', err);}else{
                                var update_query = "UPDATE grid_osm SET water = 0 WHERE id IN (";
                                for(var i = 0; i<result.rows.length; i++){
                                    if(i>0){
                                        update_query += ",";
                                    }
                                    update_query += result.rows[i].gid;
                                }
                                update_query += ")";
                                client.query(update_query, [], function(err, r) {
                                    if(err){console.log((new Date()), 'error running query', err);}else{
                    ...
                    case "green":
                        ...
                        var query = "SELECT "+
                                    ...
                                    greens_query+
                                    " ) AND way IS NOT NULL "+
                                    " ORDER BY way <-> ST_SetSRID(ST_MakePoint("+result.rows[0].longitude+","+result.rows[0].latitude+"), 900913) LIMIT 1";
...
function setRoads(level, res){
    ...
    var query = "SELECT "+
                " ST_Distance(Geography(ST_Transform(ST_SetSRID(way, 900913),4326)), Geography(ST_Transform(ST_SetSRID(ST_MakePoint("+res.longitude+","+res.latitude+"),900913),4326))) AS dist "+
                " FROM planet_osm_roads WHERE ( "+
                roads_query+
                " ) AND way IS NOT NULL "+
                " ORDER BY way <-> ST_SetSRID(ST_MakePoint("+res.longitude+","+res.latitude+"), 900913) LIMIT 1";
...
function setTransport(level, res){
    ...
    for(var i = 0; i<transport[level].length; i++){
        ...
        transport_query += "\""+transport[level][i][0]+"\" = '"+transport[level][i][1]+"'";
        if(transport[level][i].length===3){
            transport_query += " AND \""+transport[level][i][2][0][0]+"\" = '"+transport[level][i][2][0][1]+"' ) ";
        }
    }
