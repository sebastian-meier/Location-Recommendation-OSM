...
client.connect(function(err) {
	if(err) {
        ...
        client.query("SELECT COUNT(*) AS c FROM venues", [], function(err, result) {
...
function processLocations(){
    client.query("SELECT id, "+ptype+" AS val FROM venues_osm WHERE "+ptype+" = 999999 LIMIT 1", [], function(err, result) { // OFFSET "+loc_count
        if(err){console.log((new Date()), 'error running query', err);}else{
                ...
                switch(ptype){
                    ...
                    case "water":
                        ...
                        var query = "WITH dist_query AS ("+
                            ...
                            "  FROM venues_osm, planet_osm_polygon "+
                            "   WHERE id = "+loc_id+
                        ...
                        client.query(query, [], function(err, result){
                            if(err){console.log((new Date()), 'error running query', err);}else{
                                ...
                                if(result.rows.length<1){
                                    ...
                                    client.query("UPDATE venues_osm SET water = 5001 WHERE id = "+loc_id, [], function(err, r) {
                                        if(err){console.log((new Date()), 'error running query', err);}else{
                                            console.log("too far");
                                            nextLocation();
                                        }
                                    ...
                                    client.query("UPDATE venues_osm SET water = "+parseFloat(result.rows[0].dist)+" WHERE id = "+loc_id, [], function(err, r) {
                                        if(err){console.log((new Date()), 'error running query', err);}else{
                        ...
                        client.query(query, [], function(err, result){
                            if(err){console.log((new Date()), 'error running query', err);}else{
                                ...
                                if(result.rows.length<1){
                                    ...
                                    client.query("UPDATE venues_osm SET green = 5001 WHERE id = "+loc_id, [], function(err, r) {
                                        if(err){console.log((new Date()), 'error running query', err);}else{
                                            console.log("too far");
                                            nextLocation();
                                        }
                                    ...
                                    client.query("UPDATE venues_osm SET green = "+parseFloat(result.rows[0].dist)+" WHERE id = "+loc_id, [], function(err, r) {
                                        if(err){console.log((new Date()), 'error running query', err);}else{
                                            console.log(parseFloat(result.rows[0].dist));
                                            nextLocation();
                                        }
...
function setRoads(level, res){
    ...
    var query = "WITH dist_query AS ("+
        "  SELECT "+
        "    ST_Distance(way, geom) AS dist"+
        "  FROM venues_osm, planet_osm_roads "+
        "   WHERE id = "+loc_id+
    ...
    client.query(query, [], function(err, result){
        if(err){console.log((new Date()), 'error running query', err);}else{
            ...
            if(result.rows.length<1){
                ...
                client.query("UPDATE venues_osm SET road_l"+(level+1)+" = 10001 WHERE id = "+loc_id, [], function(err, r) {
                    if(err){console.log((new Date()), 'error running query', err);}else{
                        level++;
                ...
                client.query("UPDATE venues_osm SET road_l"+(level+1)+" = "+parseFloat(result.rows[0].dist)+" WHERE id = "+loc_id, [], function(err, r) {
                    if(err){console.log((new Date()), 'error running query', err);}else{
                        level++;
                        if(level===roads.length){
                            console.log(parseFloat(result.rows[0].dist));
...
function setTransport(level, res){
    ...
    var query = "WITH dist_query AS ("+
        "  SELECT "+
        "    ST_Distance(way, geom) AS dist"+
        "  FROM venues_osm, planet_osm_point "+
        "   WHERE id = "+loc_id+
    ...
    client.query(query, [], function(err, result){
        if(err){console.log((new Date()), 'error running query', err);}else{
            ...
            if(result.rows.length<1){
                ...
                client.query("UPDATE venues_osm SET "+(transport_types[level])+" = 5001 WHERE id = "+loc_id, [], function(err, r) {
                    if(err){console.log((new Date()), 'error running query', err);}else{
                        level++;
                ...
                client.query("UPDATE venues_osm SET "+(transport_types[level])+" = "+parseFloat(result.rows[0].dist)+" WHERE id = "+loc_id, [], function(err, r) {
                    if(err){console.log((new Date()), 'error running query', err);}else{
                        level++;
                        if(level===transport.length){
                            console.log(parseFloat(result.rows[0].dist));
